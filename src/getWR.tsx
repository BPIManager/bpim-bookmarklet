const main = class main {
  ...
  async getWRs() {
    const r = await fetch(
      `https://p.eagate.573.jp/game/2dx/29/ranking/json/topranker.html?series_id=${this.iidxVer}`
    );
    if (!r.ok) return;
      ...
      const check = (body: any) => {
        ...
        if (target.length > 0) {
          for (let i = 0; i < target.length; ++i) {
            const targetIdx = body.findIndex(
              (_idx: any) =>
                _idx.title === target[i].title &&
                _idx.difficulty === target[i].difficulty
            );
            if (Number(target[i].difficulty) === 3) {
              //hyper
            ...
            if (Number(target[i].difficulty) === 4) {
              //another
            ...
            if (Number(target[i].difficulty) === 10) {
              //leggendaria
