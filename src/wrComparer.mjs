...
const main = class{
  ...
  loadDef = async()=>{
    this.def = {};
    const t = await (await fetch("https://proxy.poyashi.me/?type=bpi")).json();
    //this.newDef = t.body;
    t.body.filter((item)=>item.difficultyLevel == "12" && item.dpLevel == "0").map((item)=>{
      const suffix = ()=>{
        return item.difficulty === "3" ? "[H]" : item.difficulty === "4" ? "" : "[L]";
      }
  ...
  async splitSongs(text){
    const df = {
      '"Anisakis -somatic mutation type ""Forza""[L]"':"Anisakis -somatic mutation type\"Forza\"-",
      "ASIAN VIRTUAL REALITIES(MELTING TOGETHER IN DAZZLING DARKNESS)":"ASIAN VIRTUAL REALITIES (MELTING TOGETHER IN DAZZLING DARKNESS)",
      ...
      "\"Life Is A Game ft.DD\"\"ナカタ\"\"Metal\"":"Life Is A Game ft.DD\"ナカタ\"Metal",
      "Marie Antoinette":"Marie Antoinette[L]",
      "NEW GENERATION -もう、お前しか見えない-[L]":"NEW GENERATION -もう、お前しか見えない-[L]", //定義データにないよ！
    ...
    return text.split("\n").reduce((group,song)=>{
      ...
      group.push({
        ...
        updated:_new - this.def[sm[2]] > 0,
        error:!sm[3] || sm[3].indexOf("/") === -1 || !this.def[sm[2]]
      });
  ...
  async init(){
    ...
    console.log(wr);
    fs.writeFileSync('../result/result_zen1_12.json', JSON.stringify(this.newDef));
  }
}
